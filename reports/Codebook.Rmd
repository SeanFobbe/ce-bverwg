...
```{r, echo = FALSE}
tar_load(latexdefs)
tar_load(dt.bverwg.meta)
tar_load(dt.annotated)
tar_load(files.pdf)
tar_load(files.txt)
...
Dieser Datensatz ist eine digitale Zusammenstellung von möglichst allen Entscheidungen, die auf der amtlichen Internetpräsenz des Bundesverwaltungsgerichts (BVerwG) am jeweiligen Stichtag veröffentlicht waren. Die Stichtage für jede Version entsprechen exakt der Versionsnummer.
...
Die Personendaten stammen aus folgendem Datensatz: \enquote{Seán Fobbe and Tilko Swalve (2023). Presidents and Vice-Presidents of the Federal Courts of Germany (PVP-FCG). Version 2023-03-21. Zenodo. \url{https://github.com/SeanFobbe/pvp-fcg}}.
...
```{r, CE-BVerwG_00_Pipeline_Graph, fig.width = 12, fig.height = 14, fig.pos = "p", fig.cap = "Der vollständige Prozess der Datensatz-Kompilierung."}
...
4. Erst ab dem Jahr 2002 sind begründete Entscheidungen des BVerwG einigermaßen vollständig veröffentlicht (\emph{temporal bias}). Die Frequenztabellen geben hierzu genauer Auskunft.
5. Folgende Entscheidungen veröffentlicht das BVerwG in der Regel *nicht*: Einstellungsbeschlüsse, Ruhensbeschlüsse, Entscheidungen über Prozesskostenhilfe, Beiordnungsbeschlüsse, Streitwertbeschlüsse, Kostenfestsetzungsbeschlüsse, Beiladungen, Anhörungsrügen, Vergleiche, Verwerfungen von Beschwerden zum Bundesverwaltungsgericht nach § 152 VwGO, Entscheidungen, die dem Geheimschutz unterliegen oder die durch die gesetzlich vorgeschriebene Anonymisierung unverständlich oder verfälscht werden.
...
Alle hinzugefügten Metadaten sind vollständig maschinenlesbar dokumentiert. Sie sind entweder im Source Code enthalten, mit dem Source Code zusammen dokumentiert oder über dauerhaft stabile Identifikatoren (z.B. DOI) zitiert.
...
PDF & \textbf{Traditionelle juristische Forschung}. Die PDF-Dokumente wie sie vom BVerwG auf der amtlichen Webseite bereitgestellt werden, jedoch verbessert durch semantisch hochwertige Dateinamen, die der leichteren Auffindbarkeit von Entscheidungen dienen. Die Dateinamen sind so konzipiert, dass sie auch für die traditionelle qualitative juristische Arbeit einen erheblichen Mehrwert bieten. Im Vergleich zu den CSV-Dateien enthalten die Dateinamen nur einen reduzierten Umfang an Metadaten, um Kompatibilitätsprobleme zu vermeiden und die Lesbarkeit zu verbessern.\\
...
- CSV-Datei mit Metadaten einlesen
- Anhand der Metadaten die gewünschte Teilmenge der Dokumente bilden
- CSV-Datei mit Linguistischen Annotationen einlesen
- Die Werte der Variable \enquote{doc\_id} der Teilmenge nutzen um aus den annotierten Daten nur diejenigen herauszufiltern, deren Variable \enquote{doc\_id} mit der Teilmenge übereinstimmt 
...
```{r}
kbl(az.brd[stelle == "BVerwG" & position == "hauptzeichen",
...
table.praesi.daten <- presidents[court == "BVerwG",
                                 .(name_last,
                                   name_first,
                                   term_begin_date,
                                   term_end_date,
...
```{r}
table.praesi.alter <- presidents[court == "BVerwG",
                                 .(name_last,
                                   name_first,
                                   term_begin_age,
                                   term_end_age,
...
```{r}
table.vpraesi.daten <- vpresidents[court == "BVerwG",
                                 .(name_last,
                                   name_first,
                                   term_begin_date,
                                   term_end_date,
...
```{r}
table.vpraesi.alter <- vpresidents[court == "BVerwG",
                                 .(name_last,
                                   name_first,
                                   term_begin_age,
                                   term_end_age,
...
```{r, CE-BVerwG_09_Density_Zeichen, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverwg.meta)+
...
```{r,  CE-BVerwG_10_Density_Tokens, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverwg.meta)+
...
```{r, CE-BVerwG_11_Density_Typen, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverwg.meta)+
...
```{r, CE-BVerwG_12_Density_Saetze, fig.height = 6, fig.width = 9}
ggplot(data = dt.bverwg.meta)+
...
dt.summary.docvars <- dt.bverwg.meta[,
...
dt.unique.docvars <- dt.bverwg.meta[,
...
```{r}
freqtable.etyp <- f.fast.freqtable(dt.bverwg.meta,
...
```{r, CE-BVerwG_02_Barplot_Entscheidungstyp, fig.height = 6, fig.width = 9}
...
freqtable.az <- f.fast.freqtable(dt.bverwg.meta,
...
```{r, CE-BVerwG_03_Barplot_Spruchkoerper_AZ, fig.height = 6, fig.width = 9}
...
freqtable.regz <- f.fast.freqtable(dt.bverwg.meta,
...
```{r, CE-BVerwG_04_Barplot_Registerzeichen, fig.height = 8, fig.width = 10}
...
freqtable.praesi <- f.fast.freqtable(dt.bverwg.meta,
                                 varlist = "praesi",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
...
```{r, CE-BVerwG_05_Barplot_PraesidentIn, fig.height = 6, fig.width = 9}
...
freqtable.vpraesi <- f.fast.freqtable(dt.bverwg.meta,
                                 varlist = "v_praesi",
                                 sumrow = TRUE,
                                 output.list = TRUE,
                                 output.kable = FALSE,
...
```{r, CE-BVerwG_06_Barplot_VizePraesidentIn, fig.height = 6, fig.width = 9}
...
freqtable.entscheidungsjahr <- f.fast.freqtable(dt.bverwg.meta,
...
```{r, CE-BVerwG_07_Barplot_Entscheidungsjahr, fig.height = 6, fig.width = 9}
...
```{r}
freqtable.eingangsjahr <- f.fast.freqtable(dt.bverwg.meta,
...
```{r, CE-BVerwG_08_Barplot_EingangsjahrISO, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerwG_13_Density_Dateigroessen_PDF, fig.height = 6, fig.width = 9}
...
```{r, CE-BVerwG_14_Density_Dateigroessen_TXT, fig.height = 6, fig.width = 9}
